...
function changeTimerValue(timerField, timerFinish) {
  ...
  if (timeRemain <= 0) {
    timerField.textContent = "00:00"; //let timerId =setTimeout(function(){alert('444')}, 1000)
    //http://localhost:3000/test_passages/156/result

    serverused = window.location.host;
    var newurl = window.location.protocol + '//' + serverused + '/test_passages/' + '/result'; //newurl = "#{window.location.protocol}//#{serverused}/sites/#{node.id}/edit"
    //alert(@test_passage.id)

    return; //windows.location = result_test_passage_path(@test_passage)
  }
}
...
/******/ });
//# sourceMappingURL=application-1032014eac087091b86b.js.map